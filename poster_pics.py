...
out_dir = '/home/luciano.andrian/doc/salidas/ENSO_IOD/regression/Full/poster_son/'
...
def PlotReg(data, data_cor, levels=np.linspace(-100,100,2), cmap='RdBu_r'
            ...
            ,  sig2=False, step=1,SA=False, contour0=False, color_map = '#d9d9d9'):
...
variables = ['pp_gpcc','t_cru','hgt200','div']
interp = [False, False,  False]
seasons = [10] # main month

var_name = ['var', 'var', 'z', 'div']
title_var = ['PRATE','T2m', 'HGT200', 'Divergence']

two_variables = [True, True, False, False]
SA = [True,  True, False, True]
step = [1,1,1, 3]
sig = [True, True, True, True]

scales = [np.linspace(-15, 15, 13),  # pp
          np.linspace(-0.6, 0.6, 13),  # t
          np.linspace(-150, 150, 13), #hgt
          np.linspace(-0.15e-5,0.15e-5,13)]
...
for v in variables:
    ...
    # Open: -------------------------------------------------------------------------------------------------------#
    if (v != variables[0]) & (v != variables[1]):
...
def PlotEnso_Iod(dmi, ninio, dmi_or, ninio_or, title="", fig_name = 'fig_enso_iod', out_dir=out_dir, save=False):
    from numpy import ma
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots()

    plt.scatter(x=dmi_or, y=ninio_or, marker='o', s=20, edgecolor='k', color='gray', alpha=0.3)
    im = plt.scatter(x=dmi, y=ninio, marker='o', s=40, edgecolor='k', color='orangered')
    ...
    plt.text(-3.8, 3.4, 'EN/IOD-', dict(size=15))
    plt.text(-.2, 3.4, 'EN', dict(size=15))
    plt.text(+2.3, 3.4, 'EN/IOD+', dict(size=15))
    plt.text(+2.6, -.1, 'IOD+', dict(size=15))
    plt.text(+2.3, -3.4, 'LN/IOD+', dict(size=15))
    plt.text(-.2, -3.4, 'LN', dict(size=15))
    plt.text(-3.8, -3.4, ' LN/IOD-', dict(size=15))
...
#########################################################################################################################
# indices: ------------------------------------------------------------------------------------------------------------#
dmi, dmi_2, dmi_3 = DMI(filter_bwa=False, start_per=str(i), end_per=str(end))
y_dmi = dmi.where((dmi.Mes>=9) & (dmi.Mes<=11)).Años.dropna().values
#y_dmi = dmi.where((dmi.Mes==10)).Años.dropna().values

aux = xr.open_dataset("/pikachu/datos4/Obs/sst/sst.mnmean_2020.nc")
n34, n34_2, n34_3 = Nino34CPC(aux, start=i)
y_n34 = n34_3.where((n34_3.Mes>=9) & (n34_3.Mes<=11)).Años.dropna().values
#y_n34 = n34_3.where((n34_3.Mes==10)).Años.dropna().values

y_sim = np.intersect1d(y_n34, y_dmi)

dmi_3 = dmi_3.sel(time=slice(str(i) + '-01-01', str(end) + '-12-01'))
n34 = n34.sel(time=slice(str(i) + '-01-01', str(end) + '-12-01'))

dmi = dmi_3/dmi_3.std('time')
n34 = n34/n34.std('time')

aux = dmi.sel(time=dmi.time.dt.year.isin(y_sim))
aux = aux.sel(time=is_months(month=aux['time.month'], mmin=9, mmax=11))
aux = aux.groupby('time.year').mean()

aux2 = n34.sel(time=n34.time.dt.year.isin(y_sim))
aux2 = aux2.sel(time=is_months(month=aux2['time.month'], mmin=9, mmax=11))
aux2 = aux2.groupby('time.year').mean()

dmi_or = dmi.groupby('time.season')['SON']
dmi_or = dmi_or.groupby('time.year').mean()

n34_or = n34.groupby('time.season')['SON']
n34_or = n34_or.groupby('time.year').mean()


PlotEnso_Iod(aux, aux2, dmi_or, n34_or,
             save=False)
