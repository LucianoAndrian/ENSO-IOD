...
out_dir_w_sig = '/home/luciano.andrian/doc/salidas/ENSO_IOD/CONGREMET/DMIbase/'
out_dir_no_sig = '/home/luciano.andrian/doc/salidas/ENSO_IOD/CONGREMET/DMIbase/'
...
for v in variables_t_p:
    ...
    for c in cases:
        ...
        for s in seasons:
            ...
            mask=MakeMask(comp1, dataname='var')
            mask_sig = MakeMask(sig, dataname='var')

            if v == variables_t_p[2]:
                #Enmascarar el oceano quita mucho del contienente por la resolución. 2.5
                mask = 1
                mask_sig = 1
            ...
            Plot(comp=comp1, levels=scales[v_count_sc], cmap = cmap_t_pp[v_count], step1=1, contour1=False,
                 ...
                 mapa='sEsA', significance=True,
                 title=v_name[v_count] + '\n' + title_case[c_count] + '\n' + s + ' - Events: ' + str(num_case) ,
                 name_fig=v_name_fig[v_count] + s + '_' + cases[c_count] + '_mer_d_w_DMIbase',
    ...
    for c in cases:
        ...
        for s in seasons:
            ...
            data_sig = xr.open_dataset(sig_dir + v.split('_')[0] + '_' +
                                        c + '1950_2020_' + s + '_DMIbase.nc')

            comp1_i=comp1.interp(lon=data_sig.lon.values, lat=data_sig.lat.values)
            sig = comp1_i.where((comp1_i < data_sig['var'][0]) | (comp1_i > data_sig['var'][1]))
            sig = sig.where(np.isnan(sig['var']), 0)

            Plot(comp=comp1, levels=scales[v_count], cmap=cbar_t, step1=1,
                 contour1=True, two_variables=False,
                 mapa='hs', significance=True,
                 title='HGT200' + '\n' + title_case[c_count] + '\n' + s + ' - Events: ' + str(num_case),
                 name_fig='HGT200_'  + s + '_' + cases[c_count] + '_mer_d_w_NSA_DMIbase',
                 dpi=dpi, save=save, comp_sig=sig, color_sig='k')
...
for c in cases:
    ...
    for s in seasons:
        ...
        Plot(comp=comp1, levels=[-1.5, -1, -0.5, -0.25, 0, 0.25, 0.5, 1, 1.5], cmap=cbar_sst, step1=1, contour1=False,
             ...
             title='SST' + '\n' + title_case[c_count] + '\n' + s + ' - Events: ' + str(num_case),
             name_fig='SST_' + s + '_' + cases[c_count] + '_d_NSA_DMIbase',
...
for c in cases:
    ...
    for s in seasons:
        ...
        Plot(comp=comp1, levels=np.linspace(-0.5e-5, 0.5e-5, 13), cmap=cbar_sst, step1=1, contour1=True,
             ...
             name_fig='divp_' + s + '_' + cases[c_count] + '_d_NSA_DMIbase',
