...
data = xr.open_dataset(data_dir +v + '.nc')
p40 = data.sel(lat=-10, lon=285, method='nearest')
trend = np.polyfit(range(0, len(p40['var'])), p40['var'], deg=1)
p40_d = p40['var'] - trend[0] * range(0, len(p40['var']))
plt.plot(p40['var'], color='k');plt.plot(p40_d, color='yellow');plt.show()


trend = np.polyfit(range(0, len(p65['var'])), p65['var'], deg=1)
p65 = p65['var'] - trend[0] * range(0, len(p65['var']))

sam_d = (p40 - p40.mean(dim='time')) / p40.std(dim='time') - (p65 - p65.mean(dim='time')) / p65.std(dim='time')