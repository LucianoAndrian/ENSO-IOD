"""
Composiciones de Tsigma a partir de los outputs de
...
for s in seasons:
    ...
    neutro = xr.open_dataset(
        cases_dir + 'tsigma_neutros_' + s + name_fig_end + '.nc')\
        .rename({'TMP':'var'})

    for c, c_count in zip(cases, range(0, len(cases))):
        case = xr.open_dataset(
            cases_dir + 'tsigma_' +  c + '_' + s + name_fig_end + '.nc')\
            .rename({'TMP':'var'})
        try:
            ...
            Plot(comp, levels=scale_snr, cmap=cbar_t, dpi=dpi, step=1,
                 name_fig='tsigma_comp_' + c + '_' + s + name_fig_end,
            ...
            Plot(snr, levels=scale_snr, cmap=cbar_snr, dpi=dpi, step=1,
                 name_fig='tsigma_SNR_' + c + '_' + s + '_' + name_fig_end,
