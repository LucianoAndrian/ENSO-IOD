"""
Selección y clasificación de los eventos IOD y ENSO
"""
########################################################################################################################
import xarray as xr
import numpy as np
...
for ms in main_month_season:
    ...
    #data_n34_restricted = n34_season.where(np.abs(n34_season) < 0.5*n34_season.mean('r').std())
    ...
    for r in range(1, 25):
        ...
        dmi_sim_pos, dmi_sim_neg = xrClassifierEvents(dmi_sim, r=666, by_r=False)
        n34_sim_pos, n34_sim_neg = xrClassifierEvents(n34_sim, r=666, by_r=False)
        ...
        if (len(sim_events) != (len(sim_pos) + len(sim_neg))):
            ...
            dmi_pos_n34_neg = dmi_sim.sel(time=dmi_sim.time.isin(dmi_pos_n34_neg))

            dmi_neg_n34_pos = np.intersect1d(dmi_sim_neg, n34_sim_pos)
            dmi_neg_n34_pos = dmi_sim.sel(time=dmi_sim.time.isin(dmi_neg_n34_pos))
        else:
            dmi_pos_n34_neg = []
            dmi_neg_n34_pos = []
        ...
        dmi_puros_pos, dmi_puros_neg = xrClassifierEvents(dmi_puros, r=666, by_r=False)
        n34_puros_pos, n34_puros_neg = xrClassifierEvents(n34_puros, r=666, by_r=False)

        # Años neutros. Sin ningun evento.
        """
        Un paso mas acá para elimiar las fechas q son nan debido a dato faltante del CFSv2
        En todo el resto del código no importan xq fueron descartados todos los nan luego de 
        ...
        mask = np.in1d(dates_ref, dmi.time, invert=True) #cuales de esas fechas no fueron dmi
        neutros = dmi_season.sel(time=dmi_season.time.isin(dates_ref.time[mask]), r=r)

        mask = np.in1d(neutros.time, n34.time, invert=True)  # cuales de esas fechas no fueron n34
