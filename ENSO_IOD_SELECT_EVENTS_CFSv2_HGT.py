...
def xrFieldTimeDetrend_HGT(xrda, dim, deg=1):
    # detrend along a single dimension
    aux = xrda.polyfit(dim=dim, deg=deg)
    trend = xr.polyval(xrda[dim], aux.hgt_polyfit_coefficients[0])# + aux.hgt_polyfit_coefficients[1]
    dt = xrda - trend
    return dt
########################################################################################################################
...
for m_name in mmonth_seasons_names:
    ...
    data_hgt_s = xrFieldTimeDetrend_HGT(data_hgt_s, 'time')
