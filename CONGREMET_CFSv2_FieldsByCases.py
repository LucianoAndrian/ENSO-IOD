...
(Similar a *2D_bins_DMI_N34_3.0.py, pero en lugar de regiones para all SA)
"""
########################################################################################################################
...
def BinsByCases(v, v_name, fix_factor, s, mm, c, c_count,
    ...
    if neutro_clim:
        ...
        clim = clim.sel(lon=slice(295, 315), lat=slice(-40, -12))
    ...
    case = case.sel(lon=slice(295, 315), lat=slice(-40, -12))
...
def ComputeFieldsByCases(v, v_name, fix_factor, snr, data,
    ...
    for s in (['SON']):
        ...
        for c_count in [0, 1, 2, 3, 4, 5, 6, 7]:  # , 8]: # Loop en los cases -{neutro}
            ...
            for b_dmi in range(0, len(bins_aux_dmi)):
                for b_n34 in range(0, len(bins_aux_n34)):
                    ...
                    #comp_case = comp_case.sel(lon=slice(295, 330), lat=slice(-60, 15))
                    ...
                    if num_bin[b_dmi][b_n34] > num_neutros[mm-7]*porcentaje:
                        ...
                        axs[n].add_feature(cartopy.feature.BORDERS, alpha=0.7)
                        if proj == 'eq':
        ...
        axs[12].add_feature(cartopy.feature.BORDERS, alpha=0.7)
        if v_name != 'hgt':
...
#data = data.sel(lon=slice(295, 315), lat=slice(-40, -12))
