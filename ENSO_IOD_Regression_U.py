...
SA = False
sig = True
scale = [-3.5, -2.5, -1.5, -0.5, -.25, 0, .25, .5, 1.5, 2.5, 3.5]
...
# data: ---------------------------------------------------------------------------------------------------------------#
data_u = xr.open_dataset('/pikachu/datos/luciano.andrian/observado/ncfiles/ERA5/mer_d_w/ERA5_U_mer_d_w.nc')
time_original = data_u.time
...
data_u = data_u.groupby('time.month') - data_u.groupby('time.month').mean('time', skipna=True)

# 3-month running mean ------------------------------------------------------------------------------------------------#
# data_div = data_div.rolling(time=3, center=True).mean()
# data_vp = data_vp.rolling(time=3, center=True).mean()
data_u = data_u.rolling(time=3, center=True).mean()
data_u_200 = data_u.sel(level=200)
s_count=0
for s in seasons_name:
    aux_n34, aux_corr_n34, aux_dmi, \
    aux_corr_dmi, aux_n34_2, aux_corr_n34_2, \
    aux_dmi_2, aux_corr_dmi_2 = ComputeWithEffect(data=data_u_200, data2=None, n34=n34, dmi=dmi,
                                                  two_variables=two_variables, m=seasons[s_count],
                                                  full_season=False, time_original=time_original)
    v_count=0
    print('Plot')
    PlotReg(data=aux_n34, data_cor=aux_corr_n34*SigMask(aux_n34, 0.75),
    ...
    PlotReg(data=aux_dmi, data_cor=aux_corr_dmi*SigMask(aux_dmi, 0.75),
            levels=scale, cmap=cbar, dpi=dpi,
            title='U' + '\n' + s +
                          ' - ' + str(p[0]) + '-' + str(p[1]) + ' DMI',
    ...
    aux_n34_wodmi, aux_corr_n34, aux_dmi_won34, aux_corr_dmi = \
        ComputeWithoutEffect(data_u_200, n34, dmi, seasons[s_count], time_original)


    PlotReg(data=aux_n34_wodmi, data_cor=aux_corr_n34*SigMask(aux_n34_wodmi, 0.75),
            levels=scale, cmap=cbar, dpi=dpi,
            title='U' + '\n' + s +
    ...
    PlotReg(data=aux_dmi_won34, data_cor=aux_corr_dmi*SigMask(aux_dmi_won34, 0.75),
            levels=scale, cmap=cbar, dpi=dpi,
            title='U' + '\n' + s +
