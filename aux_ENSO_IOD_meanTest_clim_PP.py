"""
t-test de medias para las climatologias de las variables PP
"""
########################################################################################################################
import xarray as xr
import numpy as np
...
def Plot(data_plot, data_var, levels = np.linspace(-3,3,11), cmap='RdBu',
    ...
    ax.set_xticks(np.arange(275, 330, 5), crs=crs_latlon)
    ax.set_yticks(np.arange(-60, 15, 5), crs=crs_latlon)
...
out_dir = '/home/luciano.andrian/doc/salidas/ENSO_IOD/Modelos/Composites/PP/diff_clim/'
v = 'prec'
...
t_cr=t.ppf(0.95, aux_df.prec)
aux = t_jja.where(abs(t_jja) > t_cr)

for l in [0,1,2,3]:
    Plot(aux, aux.prec[l, :, :], cmap='RdBu_r', title='t JJA - Lead ' + str(l) + ' - PP',
         name_fig='PP_t_test_jja_' + str(l), save=save)
...
for l in [0,1,2,3]:
    Plot(aux, aux.prec[l, :, :], cmap='RdBu_r', title='t JAS- Lead ' + str(l) + ' - PP',
         name_fig='PP_t_test_jas_' + str(l), save=save)
...
for l in [0,1,2,3]:
    Plot(aux, aux.prec[l, :, :], cmap='RdBu_r', title='t ASO- Lead ' + str(l) + ' - PP',
         name_fig='PP_t_test_aso_' + str(l), save=save)
...
for l in [0,1,2,3]:
    Plot(aux, aux.prec[l, :, :], cmap='RdBu_r', title='t SON- Lead ' + str(l) + ' - PP',
         name_fig='PP_t_test_son_' + str(l), save=save)
