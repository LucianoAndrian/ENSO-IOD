...
def open_mfdataset_merge_only(paths):
    """
    Los .nc de real_time con xr.open_mfdataset: Resulting object does not have monotonic global indexes along dimension
...
for v in variables:
    ...
    for ms in mmonth_seasons:
        ...
        for r in range(1, 25):  # loop en los miembros de ensamble
            ...
            for path in [dir_hc, dir_rt]:
                ...
                if path==dir_hc:
                    ...
                    data.sel(X=slice(275, 330), Y=slice(-60, 15))
                elif path==dir_rt:
        ...
        data_season_r_f.to_netcdf(out_dir + seasons[ms - 7] + v +'_Leads_r_CFSv2.nc')
