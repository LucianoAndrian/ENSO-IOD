...
out_dir = '/home/luciano.andrian/doc/salidas/ENSO_IOD/Modelos/SNR_CFSv2/rest_sst/'
save=True
dpi=200
# Funciones ############################################################################################################
...
scales_snr = [[-1,-.8,-.6,-.4,-.2,-.1,0,0.1,0.2,0.4,0.6,0.8,1],[-1,-.8,-.6,-.4,-.2,-.1,0,0.1,0.2,0.4,0.6,0.8,1]]
        ...
        for s_n in set_name:
            ...
            for c in cases:
                ...
                try:
                    ...
                    # signal
                    aux = data_case.mean(['time', 'L']) - data_neutral.mean(['L','time'])
                    #print(aux)
                    aux *= mask
                    Plot(aux, levels=scales[v_count], cmap=colormap[v_count],
                         dpi=dpi, step=1,
                         name_fig=v + 'REST_SST_set_' + s_n + '_' + c + '_' + s + '.nc',
                         title='[REST. SST N34] - Mean Composite - CFSv2 - ' + s + '\n' + title_case[c_count] + '\n' + ' ' + v + ' - ' +
                               'Leads: ' + str(s_n) + ' - ' + 'Casos: ' + str(num_case),
                         save=save)

                    # spread
                    aux2 = data_case.mean(['L']) - aux
                    aux2 = aux2.std('time')
                    aux2 *= mask
                    ...
                    Plot(snr, levels=scales_snr[v_count], cmap=cbar_snr,
                         dpi=dpi, step=1,
                         name_fig=v + 'SNR_REST_SST_set_' + s_n + '_' + c + '_' + s + '.nc',
                         title='[REST. SST N34] - SNR Composite - CFSv2 - ' + s + '\n' + title_case[c_count] + '\n' + ' ' + v + ' - ' +
                               'Leads: ' + str(s_n) + ' - ' + 'Casos: ' + str(num_case),
                         save=save)
                except:
