"""
t-test de medias para las climatologias de las variables T
"""
########################################################################################################################
import xarray as xr
import numpy as np
...
t_cr=t.ppf(0.95, aux_df.tref)
aux = t_jja.where(abs(t_jja) > t_cr)

for l in [0,1,2,3]:
    Plot(aux, aux.tref[l, :, :], cmap='RdBu_r', title='t JJA - Lead ' + str(l) + ' - T',
         name_fig='T_t_test_jja_' + str(l), save=save)
...
for l in [0,1,2,3]:
    Plot(aux, aux.tref[l, :, :], cmap='RdBu_r', title='t JAS- Lead ' + str(l) + ' - T',
         name_fig='T_t_test_jas_' + str(l), save=save)
...
for l in [0,1,2,3]:
    Plot(aux, aux.tref[l, :, :], cmap='RdBu_r', title='t ASO- Lead ' + str(l) + ' - T',
         name_fig='T_t_test_aso_' + str(l), save=save)
...
for l in [0,1,2,3]:
    Plot(aux, aux.tref[l, :, :], cmap='RdBu_r', title='t SON- Lead ' + str(l) + ' - T',
         name_fig='T_t_test_son_' + str(l), save=save)
