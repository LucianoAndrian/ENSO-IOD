...
out_dir = '/home/luciano.andrian/doc/salidas/ENSO_IOD/Modelos/SNR_CFSv2/SST/SST_rest_sst/'
save=True
dpi=200
# Funciones ############################################################################################################
    ...
    for s_n in set_name:
        ...
        for c in cases:
            ...
            try:
                ...
                # signal
                aux = data_case.mean(['time','L']) - data_neutral.mean(['time','L'])
                Plot(aux, levels=np.linspace(-1.2,1.2,13), cmap=cbar_t,
                     dpi=dpi, step=1,
                     name_fig=v + 'RESTSST_set_' + s_n + '_' + c + '_' + s + '.nc',
