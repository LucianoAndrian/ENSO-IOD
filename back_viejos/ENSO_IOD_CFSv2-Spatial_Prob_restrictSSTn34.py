...
########################################################################################################################
cases_dir = '/pikachu/datos/luciano.andrian/cases/cases_restrict_SST/'
out_dir = '/home/luciano.andrian/doc/salidas/ENSO_IOD/Modelos/SpatialProb/SP_rest_sst/'
save=True
dpi=200
# Funciones ############################################################################################################
def Plot(comp, levels = np.linspace(-1,1,11), cmap='RdBu',
         dpi=200, save=True, step=1,
...
def SpatialProbability(data, mask):
    ...
    for ln in range(0, 56):
        for lt in range(0, 76):
            prob['var'][lt, ln] = \
                len(data[:, lt, ln][~np.isnan(data[:, lt, ln].values)].values) \
                / len(data[:, lt, ln])
    return prob*mask
########################################################################################################################
variables = ('prec', 'tref')
...
# cbar_pp = colors.ListedColormap(['#FFFFFF','#FFFFFF','#FFFDAB', '#FFDB57',
#                                  '#FF9F35', '#FF7603','#D62B35', '#971E25', '#970090', '#6D0068'])
cbar_pp = colors.ListedColormap(['#6E2900', '#6E4007', '#A5691B','#CFA256','#EAD59F','#F5EFDC','#FFFFFF','#FFFFFF',
                                 '#DEF0ED','#A3DBD3', '#58B0A7', '#1A7E76', '#005046', '#004050'])

cbar_pp.set_bad(color='white')


cbar_t =  colors.ListedColormap(['#00125E', '#124984', '#327CB7','#67A4CC','#93B5D6','#D0D1E6',
                                 '#FFFFFF','#FFFFFF',
                                 '#FFF8BB', '#FEBA55', '#FD8038', '#C43B3C', '#8A0B25','#670A40'])

cbar_t.set_bad(color='white')

colorbars_0 = [cbar_pp, cbar_t]
#colorbars_1 = ['']
...
#        for s_n in set_name:
        ...
        for c in cases:
            ...
            try:
                ...
                # reordenando para la funcion Prob
                mmonth_seasons = [7, 8, 9, 10]
                for l in [0, 1, 2, 3]:
                    for ms in [mmonth_seasons[s_count] - l]:
                        aux = data_case.sel(time=data_case['var'].time.dt.month.isin(ms), L=data_case['var'].L.isin(l))
                        aux = aux['var'][0,:,:,:]
                        if l == 0:
                            aux2 = aux
                        else:
                            aux2 = xr.concat([aux2, aux], dim='time')
                data_case = aux2
                ...
                aux2 = xr.where(data_case > 0, data_case, np.nan)
                prob = SpatialProbability(aux2, mask)
                Plot(prob, levels=[0,.1,.2,.3,.4,.45,.5,.55,.6,.7,.8,.9,1], cmap=colorbars_0[v_count],
                     dpi=dpi, step=1,
                     name_fig=v + 'REST_SST_set_' + s_n + '_+0SD_' + c + '_' + s + '.nc',
                     title='[REST. SST N34] Spatial Probability - CFSv2 - ' + s + '\n' + title_case[c_count] + '\n' + ' ' +
                           v + '>0' + ' - ' + 'Leads: ' + s_n, save=save)
            except:
                x = None
                print('No')
