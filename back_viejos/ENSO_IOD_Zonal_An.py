...
cmap = [cbar, cbar, cbar, cbar_r, cbar_r, cbar, cbar, 'BrBG']
contour0 = [True, False, True, True, True, True, True, True]
# Functions ############################################################################################################
...
for v in variables:
    ...
    if (v == variables[1]) | (v == variables[2]) | (v == variables[5]) | (v == variables[6]) | (v == variables[7]):
        print('1x1')
        data = xr.open_dataset(data_dir + v + '1x1.nc')
        zonalAnom = data - data.mean(dim='lon')
    else:
    ...
    for s in seasons:
        ...
        neutro_comp_za = CompositeSimple(original_data=zonalAnom, index=neutro,
                                      mmin=mmin, mmax=mmax)
        # composite and
        #
        ...
        for y in dmi_un_neg.values:
            ...
            #      SA=SA[c_var], dpi=200, save=save, step=step[c_var],
            #      name_fig=v + '_DMI_un_neg_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
            #      title=str(int(y)) + ' - Negative Isolated DMI - ' + v_name[c_var] + '\n' + s + ' ' + i + '-2020',
            #      contour0=contour0[c_var])

            comp = comp - comp.mean(dim='lon')
            Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
                 SA=SA[c_var], dpi=200, save=save, step=step[c_var],
                 name_fig=v + 'ZA_of_Comp_DMI_un_neg_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
                 title=str(int(y)) + ' Zonal Anom. of Composite' + '\n' + 'Negative Isolated DMI - ' +
                       v_name[c_var] + '\n' + s + ' ' + i + '-2020',
                 contour0=contour0[c_var])

            #composite of zonalAnom. da lo mismo...
            # data_y = CompositeSimple(original_data=zonalAnom, index=[y],
            #                          mmin=mmin, mmax=mmax)
            # comp = data_y - neutro_comp_za
            #
            # Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
            #      SA=SA[c_var], dpi=200, save=save, step=step[c_var],
            #      name_fig=v + 'Comp_of_ZA_DMI_un_neg_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
            #      title=str(int(y))  + ' Composite of Zonal Anom.' + '\n'  +' Negative Isolated DMI - ' +
            #            v_name[c_var] + '\n' + s + ' ' + i + '-2020',
            #      contour0=contour0[c_var])
        ...
        for y in dmi_un_pos.values:
            ...
            # Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
            #      SA=SA[c_var], dpi=200, save=save, step=step[c_var],
            #      name_fig=v + '_DMI_un_pos_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
            ...
            Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
                 SA=SA[c_var], dpi=200, save=save, step=step[c_var],
                 name_fig=v + 'ZA_of_Comp_DMI_un_pos_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
                 title=str(int(y)) + ' Zonal Anom. of Composite' + '\n' + 'Positive Isolated DMI - ' +
                       v_name[c_var] + '\n' + s + ' ' + i + '-2020',
                 contour0=contour0[c_var])
        ...
        for y in dmi_n34_sim_neg.values:
            ...
            # Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
            #      SA=SA[c_var], dpi=200, save=save, step=step[c_var],
            #      name_fig=v + '_DMI_SIM_neg_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
            ...
            Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
                 SA=SA[c_var], dpi=200, save=save, step=step[c_var],
                 name_fig=v + 'ZA_of_Comp_DMI_SIM_neg_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
                 title=str(int(y)) + ' Zonal Anom. of Composite' + '\n' + 'Negative DMI and ENSO - ' +
                       v_name[c_var] + '\n' + s + ' ' + i + '-2020',
                 contour0=contour0[c_var])
        ...
        for y in dmi_n34_sim_pos.values:
            ...
            # Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
            #      SA=SA[c_var], dpi=200, save=save, step=step[c_var],
            #      name_fig=v + '_DMI_SIM_pos_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
            ...
            Plot(comp=comp, levels=scales[c_var], cmap=cmap[c_var],
                 SA=SA[c_var], dpi=200, save=save, step=step[c_var],
                 name_fig=v + 'ZA_of_Comp_DMI_SIM_pos_' + str(int(y)) + '_' + s + '_' + i + '_2020.jpg',
                 title=str(int(y)) + ' Zonal Anom. of Composite' + '\n' + 'Positive DMI and ENSO - ' +
                       v_name[c_var] + '\n' + s + ' ' + i + '-2020',
                 contour0=contour0[c_var])
