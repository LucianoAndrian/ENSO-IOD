...
En este caso restringiendo más la SST de Niño3.4 en la elección de DMI_un
Niño3.4 continua con el mismo crietrio
"""
########################################################################################################################
import xarray as xr
import numpy as np
...
# def xrFieldTimeDetrend_T_PP(xrda, dim, deg=1, temp=True):
...
#
#     if temp:
#         trend_f = xr.polyval(xrdaL[dim], trend.tref_polyfit_coefficients[0])# + aux.hgt_polyfit_coefficients[1]
#     else:
#         trend_f = xr.polyval(xrdaL[dim], trend.prec_polyfit_coefficients[0])  # + aux.hgt_polyfit_coefficients[1]
#
#     return trend_f
########################################################################################################################
...
for m_name in mmonth_seasons_names:
    ...
    data_tref_s['L'] = [0,1,2,3]
    data_prec_s['L'] = [0, 1, 2, 3]

    #Filtrando tendencia para cada Lead
    for l in [0,1,2,3]:
        for ms in [mmonth_seasons[m_name]-l]:
            aux_tref = data_tref_s.sel(time=data_tref_s.time.dt.month.isin(ms), L=data_tref_s.L.isin(l))
            aux_tref = aux_tref.drop('L')
            aux_prec = data_prec_s.sel(time=data_prec_s.time.dt.month.isin(ms), L=data_prec_s.L.isin(l))
            aux_prec = aux_prec.drop('L')

            # #Calculando la tendencia de la media del ensamble (para cada lead)
            aux_tref_mean = aux_tref.mean('r')
            aux1_tref = aux_tref_mean.polyfit(dim='time', deg=1, full=True)
            trend_tref = xr.polyval(coord=aux_tref.time, coeffs=aux1_tref.tref_polyfit_coefficients)
            aux3_tref = aux_tref - trend_tref

            aux_prec_mean = aux_prec.mean('r')
            aux1_prec = aux_prec_mean.polyfit(dim='time', deg=1, full=True)
            trend_prec = xr.polyval(coord=aux_prec.time, coeffs=aux1_prec.prec_polyfit_coefficients)
            aux3_prec = aux_prec - trend_prec

            if l == 0:
                aux2_tref = aux3_tref
                aux2_prec = aux3_prec
            else:
                aux2_tref = xr.concat([aux2_tref, aux3_tref], dim='L')
                aux2_prec = xr.concat([aux2_prec, aux3_prec], dim='L')

    data_prec_s = aux2_prec
    data_tref_s = aux2_tref


    # aux = xrFieldTimeDetrend_T_PP(data_prec_s, 'time', temp=False)
    # data_prec_s = data_prec_s.groupby('L') - aux
    #
    # aux = xrFieldTimeDetrend_T_PP(data_tref_s, 'time', temp=True)
    # data_tref_s = data_tref_s.groupby('L') - aux
    ...
    for s in sets:
        ...
        if len(DMI_sim_neg_N34_pos) != 0:
            ...
            data_sim_DMIneg_N34pos_f_tref.to_netcdf(
                out_dir + 'tref_' + seasons[m_name] + '_Set' + str(s[-1]) + '_sim_DMIneg_N34pos.nc')
